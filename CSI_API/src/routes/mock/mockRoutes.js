...
function generateMockServerData() {
    ...
    var data = JSON.parse(JSON.stringify(baseServerData));
    if ((_a = data.sensors) === null || _a === void 0 ? void 0 : _a.cpus) {
        Object.values(data.sensors.cpus).forEach(function (cpu) {
            cpu.utilization_percent = Number(rand(5, 95).toFixed(1));
            var utilizationDecimal = cpu.utilization_percent / 100;
            cpu.current_rate_hz = Math.floor(cpu.max_rate_Hz * utilizationDecimal);
            var baseTemp = 25;
            var maxTempIncrease = 100;
            cpu.temperature_c = Number((baseTemp + utilizationDecimal * maxTempIncrease).toFixed(1));
        });
    }
    if ((_b = data.sensors) === null || _b === void 0 ? void 0 : _b.drives) {
        Object.values(data.sensors.drives).forEach(function (drive) {
            drive.utilization_percent = Number(rand(0, 80).toFixed(2));
        });
    }
    if ((_c = data.sensors) === null || _c === void 0 ? void 0 : _c.nics) {
        Object.values(data.sensors.nics).forEach(function (nic) {
            if (nic.max_speed_bps) {
                nic.current_speed_bps = Math.floor(rand(0, nic.max_speed_bps));
            }
        });
    }
    if ((_d = data.sensors) === null || _d === void 0 ? void 0 : _d.ram) {
        data.sensors.ram.utilization_percent = rand(40, 99).toFixed(2);
    }
    if ((_e = data.sensors) === null || _e === void 0 ? void 0 : _e.gpus) {
        Object.values(data.sensors.gpus).forEach(function (gpu) {
            gpu.utilization_percent = Number(rand(0.3, 0.9).toFixed(2));
            gpu.temperature_c = Number(rand(30, 80).toFixed(1));
        });
    }
    if ((_f = data.sensors) === null || _f === void 0 ? void 0 : _f.fans) {
        Object.values(data.sensors.fans).forEach(function (fan) {
    ...
    if ((_g = data.sensors) === null || _g === void 0 ? void 0 : _g.processes) {
        Object.values(data.sensors.processes).forEach(function (process) {
            ...
            process.start_time = new Date(Date.now() - Math.floor(rand(0, 3600) * 1000)).toISOString();
        });
    }
    return data;
}
...
app.get("/server", function (c) { return c.json(generateMockServerData()); });
exports.default = app;
